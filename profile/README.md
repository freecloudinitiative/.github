...
> **Empowering Cloud Native Engineers through Open Infrastructure, Self-Hosted APIs, and Hands-on DevOps.**

Welcome to the **Free Cloud Initiative (FCI)**! We are building a fully functional, self-hosted cloud platform on bare-metal Kubernetes. You can access the live platform at **[freecloudinitiative.com](https://freecloudinitiative.com)**.
...
Our ecosystem provides an alternative to expensive managed clouds. We keep compute, storage, and networking on owned hardware, wrapped in a self-hosted API and dashboard.

This project serves a dual purpose:
1. **DevOps Showcase**: Demonstrating end-to-end automation, cloud infrastructure provisioning, and container orchestration across multi-cloud environments (GCP, Azure, AWS), bare-metal servers, and edge services (Cloudflare).
...
Our platform provides standard cloud primitives directly on bare-metal hardware:

* 🚀 **Compute** — Managed engines (containers) with optional Kata `dedicated` secure runtime isolation.
* 🗄️ **Databases** — Managed PostgreSQL clusters orchestrated via CloudNativePG.
* 📦 **Object Storage** — S3-compatible buckets powered by Garage.
* 🌐 **Networks** — Virtual Private Clouds (VPCs) and firewall rules mapped to Kubernetes NetworkPolicies.
* 💻 **Terminal Access** — Secure, browser-based shell access directly into running compute engines.
* 🔐 **Identity and Access** — Comprehensive accounts, API keys, user management, quotas, and RBAC policies.

---

## 🛠️ Technology Stack

We leverage modern, industry-standard tools to build our infrastructure and applications:

* **Infrastructure & Automation**: Terraform (Cloudflare & Multi-Cloud), Ansible
* **Container Orchestration**: K3s (Bare-metal Kubernetes), Argo CD (GitOps)
* **Backend Services**: Go (Golang)
* **Frontend Console**: React 19, TypeScript, Vite, TanStack Query
* **Observability**: Prometheus, Grafana, OpenTelemetry, Loki, Tempo
* **Data & Storage**: PostgreSQL (CloudNativePG), Garage (S3 Object Storage), Longhorn
* **Networking & Edge**: Traefik, Cert-Manager, Cloudflare Zero Trust
...
│
│   # 🔒 Backend Microservices (Private Repositories)
...
* Automated bare-metal node bootstrapping, SSH hardening, firewall management (`ufw`), and dependency installation.
...
* **Infrastructure Stack**: Traefik, Cert-Manager, Prometheus, Grafana, OpenTelemetry, Loki, Kyverno, Harbor.
* **GitOps & App Deployment**: Declarative infrastructure and application manifests managed via Argo CD.
...
### 7. 🚧 Backend Microservices *(Private Repositories)*
> *Note: The following backend services are currently in internal development in private repositories and will be released open-source as they mature.*
...
* [Go](https://go.dev/doc/install) (>= 1.22.0) for backend contribution
* Node.js & npm (for Frontend contribution)
...
* How to structure reusable Ansible roles for bare-metal server configuration and Kubernetes cluster bootstrapping.
* How to design a cloud-native Kubernetes architecture with full observability (metrics, logs, traces) on ARM64 hardware (e.g., Raspberry Pi).
* How to adopt GitOps and declarative manifest management using Argo CD.
* How to build scalable microservices in Go that interact seamlessly with Kubernetes APIs.

---

## 🤝 Contributing & Feedback

Contributions, suggestions, and feedback are welcome! Whether you are fixing a typo, adding a new Ansible role, or expanding the documentation to help others learn, feel free to open an issue or submit a pull request in the respective public repositories.
